...
.
├── src/
│   ├── app/                 # Routes & pages (scanner, history, settings, etc.)
│   │   ├── api/             # API endpoints (typed, use lib/* helpers only)
│   │   ├── floor-maps/      # Floor map UI page
│   │   ├── history/         # History UI page
│   │   ├── login/           # Auth page
│   │   ├── settings/        # VLM/Service settings UI
│   │   ├── storage/         # Storage & live buffer views
│   │   └── upload/          # Upload/scan page
│   ├── components/
│   │   ├── scanner/         # Dashboard, floor-map admin & viewer
│   │   ├── ui/              # Button, Card, Input primitives
│   │   └── auth/            # Login form, logout button
│   ├── config/              # VLM model settings & mapping
│   ├── lib/                 # DB, JSON utils, OCR/VLM orchestration, settings store
│   └── types/               # Shared TS types
├── scripts/                 # bootstrap.js + OCR/barcode helpers
├── docs/                    # QA scenarios (floor-map testing)
├── data/                    # Runtime SQLite DB & map directory (generated)
├── public/                  # Static assets
├── tests/                   # API, component, e2e, helpers
├── Dockerfile               # Multi-stage build
├── docker-compose.yml       # dev/prod services with volumes
└── .github/workflows/ci.yml # CI: install, bootstrap, lint, test, build
```
